...
// ABIs (sederhana, cukup untuk fungsi yang dipakai)
const ERC20_ABI = [
...
export default function Home() {
  ...
  const handleAddLiquidity = async () => {
    ...
    try {
      ...
      await (await tokenA.approve(CONTRACTS.pool, parsedA)).wait();

      showToast("⏳", "Approve DAI...");
      await (await tokenB.approve(CONTRACTS.pool, parsedB)).wait();

      showToast("⏳", "Menambahkan Likuiditas...");
      await (await pool.addLiquidity(parsedA, parsedB, { gasLimit: 500000 })).wait();
      ...
      <div style={{ maxWidth: "450px", margin: "0 auto", backgroundColor: "#111827", borderRadius: "20px", padding: "20px", border: "1px solid #1e293b" }}>
        ...
        {activeTab === "swap" && (
          <div>
            ...
            <button onClick={handleSwap} disabled={swapLoading || !amountIn || !account} style={{ width: "100%", backgroundColor: account ? "#fbbf24" : "#4b5563", color: account ? "#000" : "#9ca3af", border: "none", padding: "12px", borderRadius: "12px", fontWeight: "bold", cursor: account ? "pointer" : "not-allowed" }}>
              {!account ? "Connect Wallet" : swapLoading ? "Memproses..." : "Tukar Token"}
            </button>
          </div>
        )}
        ...
        {activeTab === "mint" && (
          <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
            <p style={{ fontSize: "12px", color: "#9ca3af", textAlign: "center", marginBottom: "5px" }}>Ambil 10,000 token testnet gratis langsung ke dompetmu.</p>
